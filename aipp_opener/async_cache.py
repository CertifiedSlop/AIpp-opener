"""Async caching utilities for AIpp Opener."""
...
class AsyncCache(Generic[T]):
    ...
    def __init__(
        ...
        logger.debug("AsyncCache initialized: %s (ttl=%ds, file=%s)", name, ttl, self.cache_file)
...
class AsyncAppDetectionCache(AsyncCache[list[dict]]):
    """Specialized async cache for app detection results."""
    ...
    async def get_apps(self, detector_name: str) -> Optional[list[dict]]:
        """
        ...
        """
        return await self.get(detector_name)

    async def set_apps(self, detector_name: str, apps: list[dict]) -> None:
        """
        ...
        """
        await self.set(detector_name, apps)
