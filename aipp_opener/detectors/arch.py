"""Arch Linux-specific application detector."""
...
class ArchAppDetector(AppDetector):
    """Detects applications installed on Arch Linux systems."""
    ...
    def is_available(self) -> bool:
        ...
        return (
            ...
            or Path("/etc/artix-release").exists()
            or self._pacman_available()
        )

    def _pacman_available(self) -> bool:
        """Check if pacman command is available."""
        try:
            result = subprocess.run(
                ["pacman", "--version"], capture_output=True, text=True, timeout=5
            )
    ...
    def detect(self) -> list[AppInfo]:
        """Detect installed applications on Arch Linux systems."""
        # Check memory cache first
        if self._cache is not None:
            return self._cache

        # Check disk cache
        cached_apps = self.app_cache.get_apps("arch")
        if cached_apps is not None:
            logger.debug("Using cached app detection results for Arch")
            return [AppInfo(**app) for app in cached_apps]

        apps = []
        seen_executables = set()

        # Method 1: Query pacman for installed packages
        apps.extend(self._detect_from_pacman())
        ...
        self.app_cache.set_apps("arch", [app.__dict__ for app in unique_apps])

        return unique_apps

    def _detect_from_pacman(self) -> list[AppInfo]:
        """Detect apps from pacman package database."""
        apps = []
        ...
        try:
            ...
            logger.warning("Error querying pacman: %s", e)
    ...
    def refresh(self) -> None:
        ...
        logger.info("App detection cache cleared for Arch")
