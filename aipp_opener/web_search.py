...
class WebSearcher:
    ...
    def __init__(
        ...
        if config_path is None:
            config_path = (
                Path.home() / ".config" / "aipp_opener" / "search_engines.json"
            )
    ...
    def _save_custom_engines(self) -> None:
        ...
        data = {
            "engines": [
                engine.to_dict() for engine in self._custom_engines.values()
            ]
        }
    ...
    def add_custom_engine(
        ...
        # Validate URL template has {query} placeholder
